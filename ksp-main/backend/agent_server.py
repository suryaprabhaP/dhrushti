...
agent_bp.register_blueprint(calendar_bp, url_prefix="/api/calendar")
...
# ══════════════════════════════════════════════════════════════════════════════
@agent_bp.route("/api/upload_dataset", methods=["POST"])
@agent_bp.route("/api/upload_document", methods=["POST"])
@agent_bp.route("/api/upload", methods=["POST"])
def upload_dataset():
    try:
...
@agent_bp.route("/api/datasets", methods=["GET"])
def list_datasets():
...
@agent_bp.route("/api/datasets/<path:filename>", methods=["DELETE"])
...
@agent_bp.route("/api/rag_search", methods=["POST"])
def rag_search_api():
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
@agent_bp.route("/api/connect_database", methods=["POST"])
...
# ══════════════════════════════════════════════════════════════════════════════
@agent_bp.route("/api/sarvam_tts", methods=["POST"])
def sarvam_tts():
    return jsonify({"success": False, "reason": "Standalone mode: browser native TTS active"}), 200


@agent_bp.route("/api/network_graph", methods=["GET", "POST"])
...
@agent_bp.route("/api/health", methods=["GET"])
def health():
...
# ══════════════════════════════════════════════════════════════════════════════
@agent_bp.route("/api/zia/face_analytics", methods=["POST"])
...
@agent_bp.route("/api/zia/ocr", methods=["POST"])
def zia_ocr():
...
@agent_bp.route("/api/zia/identity_scanner", methods=["POST"])
...
@agent_bp.route("/api/complaints", methods=["POST"])
...
@agent_bp.route("/api/extract_metadata", methods=["POST"])
...
# Exclusive to the Standalone Chatbot. Injects all CSV datasets as context.
# ══════════════════════════════════════════════════════════════════════════════
@agent_bp.route("/standalone/chat", methods=["POST"])
def standalone_chat():
    if not request.is_json:
        return jsonify({"success": False, "error": "Request must be JSON"}), 400
