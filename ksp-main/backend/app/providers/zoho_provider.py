...
class ZohoQuickMLProvider(BaseLLMProvider):
    ...
    _TOKEN_TTL_SECONDS = 50 * 60  # Refresh every 50 min (tokens last 1 hour)
    ...
    def __init__(self):
        ...
        # Proactive startup refresh so first request always has a valid token
        if self.refresh_token and self.client_id and self.client_secret:
            try:
                self._do_refresh()
            except Exception as e:
                log.warning(f"[ZohoQuickMLProvider] Startup refresh failed (will retry on demand): {e}")
    ...
    def _token_is_stale(self) -> bool:
        ...
        if self._token_refreshed_at == 0:
            return True
        return (time.time() - self._token_refreshed_at) >= self._TOKEN_TTL_SECONDS

    def _do_refresh(self) -> Optional[str]:
        import time
        if not (self.refresh_token and self.client_id and self.client_secret):
            log.warning("[ZohoQuickMLProvider] Cannot refresh: missing credentials")
            return None
        try:
            res = requests.post(
    ...
    def _ensure_fresh_token(self):
        if self._token_is_stale():
            log.info("[ZohoQuickMLProvider] Token stale — refreshing proactively...")
            self._do_refresh()
    ...
    def complete(self, messages: List[Dict[str, str]], json_mode: bool = False, max_tokens: int = 2500) -> Tuple[str, str]:
        ...
        # Ensure token is always fresh before calling
        ...
        for attempt in range(2):
            try:
                ...
                if res.status_code == 401 and attempt == 0:
                    log.warning("[ZohoQuickMLProvider] Unexpected 401 — emergency refresh...")
        ...
        # RAG fallback
        log.info("[ZohoQuickMLProvider] Trying RAG Knowledge Base...")
        ...
        try:
            rag_res = requests.post(self.rag_endpoint, headers=self._make_headers(), json=rag_payload, timeout=25)
            if rag_res.status_code == 401:
                self._do_refresh()
                rag_res = requests.post(self.rag_endpoint, headers=self._make_headers(), json=rag_payload, timeout=25)
            if rag_res.status_code == 200:
                resp_text = rag_res.json().get("response", "")
                if resp_text:
                    lower = resp_text.strip().lower()
                    if "cannot find" not in lower and "unable to find" not in lower:
                        return resp_text, f"{self.name}_rag"
        except Exception as e:
            log.warning(f"[ZohoQuickMLProvider] RAG fallback exception: {e}")

        log.warning("[ZohoQuickMLProvider] All endpoints failed — using local synthesizer")
