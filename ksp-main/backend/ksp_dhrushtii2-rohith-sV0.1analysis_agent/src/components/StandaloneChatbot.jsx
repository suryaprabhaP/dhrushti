...
import ComplaintPortal from './StandaloneComplaintPortal';
import ChartAnalysisModal from './StandaloneChartAnalysisModal';
import DatabaseConnectorModal from './StandaloneDatabaseConnectorModal';
...
function Chatbot({ onAddDocument, divisionName = "Bengaluru Division" }) {
  const [messages, setMessages] = useState(() => {
    try {
      const saved = localStorage.getItem(`ksp_standalone_chat_history_${divisionName}`);
      if (saved) {
  ...
  const [savedSessions, setSavedSessions] = useState(() => {
    try {
      const saved = localStorage.getItem(`ksp_standalone_chat_sessions_${divisionName}`);
  ...
  const [activeSessionId, setActiveSessionId] = useState(() => {
    try {
      const saved = localStorage.getItem(`ksp_standalone_active_session_id_${divisionName}`);
      if (saved) return saved;
    } catch (e) {
      console.error("Error reading active session ID from localStorage:", e);
    }
    return `conv_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
  });

  // PERSIST CURRENT CHAT MESSAGES TO LOCALSTORAGE (SO RESPONSES NEVER VANISH ON PAGE SWITCH)
  useEffect(() => {
    try {
      if (messages && messages.length > 0) {
        localStorage.setItem(`ksp_standalone_chat_history_${divisionName}`, JSON.stringify(messages));
      }
    } catch (e) {
      console.error("Error saving chat history to localStorage:", e);
    }
  }, [messages, divisionName]);

  // PERSIST ACTIVE SESSION ID TO LOCALSTORAGE
  useEffect(() => {
    try {
      if (activeSessionId) {
        localStorage.setItem(`ksp_standalone_active_session_id_${divisionName}`, activeSessionId);
      }
    } catch (e) {
      console.error("Error saving active session ID to localStorage:", e);
    }
  }, [activeSessionId, divisionName]);
  ...
  const updateSessionStore = (currentMsgs, overrideTitle = null) => {
    ...
    setSavedSessions(prev => {
      ...
      try {
        localStorage.setItem(`ksp_standalone_chat_sessions_${divisionName}`, JSON.stringify(updated));
      } catch (e) {
  ...
  const handleDeleteSession = (e, sessionId) => {
    ...
    setSavedSessions(prev => {
      ...
      try {
        localStorage.setItem(`ksp_standalone_chat_sessions_${divisionName}`, JSON.stringify(filtered));
      } catch (err) {
  ...
  const handleClearAllSessions = () => {
    setSavedSessions([]);
    localStorage.removeItem(`ksp_standalone_chat_sessions_${divisionName}`);
    handleNewConversation();
  };
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl('/standalone/chat'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: text,
      ...
      if (data.success && data.answer) {
        // Convert LLM chart spec {type, labels, values} -> Chart.js {labels, datasets}
        let chartData = null;
        if (data.chart_data && data.chart_data.labels && data.chart_data.values) {
          chartData = {
            labels: data.chart_data.labels,
            datasets: [{
              label: data.chart_data.title || 'Crime Data',
              data: data.chart_data.values,
              backgroundColor: data.chart_data.color || '#2563eb'
            }],
            _chartType: data.chart_data.type || 'bar'
          };
        }
        ...
        const botReply = {
          ...
          user_query: text,
          agent_type: 'standalone_data_agent',
          agent_label: data.agent_label || 'Drishti Command Assistant',
          agent_icon: data.agent_icon || '🛡️',
          agent_color: data.agent_color || '#1e40af',
          offline_fallback: false,
          prompt_suggestions: [
            "Which district has the most crime cases?",
            "Show chain snatching trends by time pattern",
            "Compare IPC crimes 2024 vs 2025",
            "What are the top cyber crime types in Karnataka?"
          ]
        };
        ...
        if (data.success) {
          ...
          setMessages(prev => [
            ...prev,
            {
              id: Date.now() + '-rag-ok',
              sender: 'bot',
              text: `✅ <b>File '${data.filename}' successfully indexed into RAG Store!</b><br/>Indexed <b>${data.chunks_indexed} vector chunks</b>. You can now ask questions about this document in the chatbot!`,
              rag_used: true,
              rag_sources: [
                {
                  ...
                  passage: `Document indexed with ${data.chunks_indexed} passage embeddings.`
                }
              ]
            }
          ]);
            ...
            {messages.length <= 1 && (
              <div style={{ textAlign: 'left', padding: '8px 4px 12px 4px', animation: 'bubble-slide-up 0.4s ease-out' }}>
                <div style={{ fontSize: '1.2rem', fontWeight: 900, color: '#1e3a8a', display: 'flex', alignItems: 'center', gap: '8px', lineHeight: 1.2 }}>
                  <ShieldCheck size={22} style={{ color: '#2563eb' }} /> Drishti Command Assistant,
                </div>
              ...
              <div key={m.id} className={`chat-bubble ${m.sender}`}>
                ...
                {m.sender === 'bot' && (
                  <div style={{ marginTop: '6px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px' }}>
                    ...
                    >
                      <BarChart2 size={11} /> {visibleCharts[m.id] ? `📊 ${((chartViewTypes[m.id] || m.chart_data?._chartType || 'BAR')).toUpperCase()} Chart 🔄` : '📊 Generate Chart'}
                    </button>
                  </div>
                )}
