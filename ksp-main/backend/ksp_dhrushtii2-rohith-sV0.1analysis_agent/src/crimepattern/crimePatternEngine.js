/**
 * 🎨 KSP SENTINEL AI — CRIME PATTERN & PATTERN MUTATION INTELLIGENCE ENGINE
...
if (typeof window === 'undefined' && typeof process !== 'undefined' && process.versions && process.versions.node) {
  try {
    ...
    const csvPath = resolve(process.cwd(), 'src/crimepattern/data/crime_pattern_dataset_2000 (1).csv');
...
export function initDataset() {
  ...
  if ((!activeCsv || activeCsv.length === 0) && typeof process !== 'undefined' && process.versions && process.versions.node) {
    try {
      const fs = require('fs');
      const path = require('path');
      const csvPath = path.resolve(process.cwd(), 'src/crimepattern/data/crime_pattern_dataset_2000 (1).csv');
      if (fs.existsSync(csvPath)) {
        activeCsv = fs.readFileSync(csvPath, 'utf8');
      }
    } catch (err) {
      console.error("Node CSV fallback read error:", err);
    }
  }
...
export function processQuery(query, currentContext = {}, selectedDivision = 'Bengaluru Division') {
  ...
  const formattedText = `<b>🛡️ SENTINEL COMMAND REPORT — ${distTarget.toUpperCase()}</b><br/>` +
  ...
  return {
    ...
    response: {
      type: 'crime_pattern_stats',
      title: `Sentinel Intelligence Report — ${distTarget}`,
      data: distFP,
      text: formattedText,
      chartData: chartDataPayload
    }
