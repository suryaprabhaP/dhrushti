...
function Login({ onLogin }) {
  ...
  const handleSelectDivision = (div) => {
    if (selectedDivision.id === div.id) return;
      ...
      <div className="login-card-container">
        ...
        <div className="login-left-panel">
              ...
              return (
                <div key={div.id} className="division-accordion-group">
                  ...
                  <div
                    ...
                    style={{
                      borderColor: isSelected ? div.color : 'transparent',
                      boxShadow: isSelected ? `0 4px 15px ${div.color}20` : 'none',
                      borderBottomLeftRadius: isSelected ? '0' : '12px',
                      borderBottomRightRadius: isSelected ? '0' : '12px'
                    }}
                  >
                    <div 
                      className="division-icon-box"
                      style={{ background: isSelected ? div.gradient : 'rgba(15, 23, 42, 0.05)', color: isSelected ? '#fff' : 'var(--text-secondary)' }}
                    >
                      <IconComp size={20} />
                    </div>
                    ...
                    <div className="division-info">
                      <div className="division-top-row">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          {isSelected ? (
                            <ChevronDown size={16} style={{ color: div.color }} />
                          ) : (
                            <ChevronRight size={16} style={{ color: 'var(--text-muted)' }} />
                          )}
                        </div>
                      </div>
                      <p className="division-tag" style={{ color: div.color }}>{div.tag}</p>
                    </div>
                  </div>

                  {/* INLINE POP DOWN STATIONS LIST UNDER THE SAME DIVISION CARD */}
                  {isSelected && (
                    <div className="stations-popdown-container" style={{ borderColor: div.color }}>
                      {/* Division Head Unit */}
                      <div
                        className={`station-popdown-item head-item ${selectedUnit.name === div.headUnit.name ? 'active' : ''}`}
                        onClick={() => handleSelectUnit(div.headUnit)}
                        style={{
                          background: selectedUnit.name === div.headUnit.name ? div.gradient : '#ffffff',
                          color: selectedUnit.name === div.headUnit.name ? '#ffffff' : '#0f172a'
                        }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <Crown size={14} style={{ color: selectedUnit.name === div.headUnit.name ? '#fef08a' : '#d97706' }} />
                          <span style={{ fontWeight: 800 }}>👑 {div.headUnit.name}</span>
                        </div>
                        {selectedUnit.name === div.headUnit.name && <Check size={14} />}
                      </div>
                      ...
                      {div.stations.map((st) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              background: isUnitActive ? `${div.color}15` : 'transparent',
                              color: isUnitActive ? div.color : 'var(--text-primary)',
                              fontWeight: isUnitActive ? 700 : 500
                            }}
                          >
                            <span style={{ fontSize: '0.74rem' }}>• {st.name}</span>
                            {isUnitActive && <CheckCircle2 size={12} style={{ color: div.color }} />}
                          </div>
                        );
                      })}
                    </div>
          ...
          <div className="login-left-footer">
            <span>🔒 Encrypted SCRB Terminal • 1100+ Police Stations Active</span>
          </div>
        </div>
        ...
        <div className="login-right-panel">
          ...
          <div className="division-title-banner" style={{ borderColor: selectedDivision.color }}>
            <div className="banner-badge" style={{ background: selectedDivision.color }}>
          ...
          <form onSubmit={handleSubmit} className="login-form">
            ...
            <button
              ...
              style={{ background: selectedDivision.gradient }}
              disabled={loading}
            >
              {loading ? (
          ...
          <div className="login-security-notice">
            ...
            <span>Secured via KSP Sentinel OAuth • Local Police Station Certified Session</span>
          </div>
