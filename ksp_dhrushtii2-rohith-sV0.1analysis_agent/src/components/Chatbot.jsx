...
import ComplaintPortal from '../portals/e_complaint/ComplaintPortal';
import PoliceInitiatedComplaintPortal from '../portals/police_complaint/PoliceInitiatedComplaintPortal';
import PassportVerificationPortal from '../portals/passport_verification/PassportVerificationPortal';
...
function getOrGenerateChartData(message, overrideData) {
  if (overrideData && overrideData.labels && overrideData.labels.length > 0) {
    return overrideData;
  }
  ...
  const query = message?.user_query || message?.userQuery || '';
  const queryL = (query + '').toLowerCase();
  const textL = (text + '').toLowerCase();

  // Auto-detect best chart type
  let chartType = 'bar';
  if (/share|percent|%|distribution|category|ratio|breakdown|composition/i.test(queryL + ' ' + textL)) {
    chartType = 'pie';
  } else if (/trend|monthly|timeline|over time|yearly|annual|progression/i.test(queryL + ' ' + textL)) {
    chartType = 'line';
  }
  ...
  // Strategy 1: Bold markdown key-value pairs (**Category**: 1,234 or **District** (450))
  const boldMatches = [...text.matchAll(/\*{1,2}([^*:\n]+?)\*{1,2}\s*[:\-–]\s*([₹$]?\s*[0-9,]+(?:\.[0-9]+)?)/g)];
  for (const [, lbl, val] of boldMatches) {
    const cleanL = lbl.replace(/[^\w\s-]/g, '').trim();
    const num = parseInt(val.replace(/[^0-9]/g, ''), 10);
    if (cleanL && cleanL.length > 1 && num > 0 && !labels.includes(cleanL)) {
      labels.push(cleanL.slice(0, 24));
      values.push(num);
    }
    if (labels.length >= 7) break;
  }

  // Strategy 2: List items (• Property Offense: 420 or 1. Theft: 150)
  if (labels.length < 2) {
    const listMatches = [...text.matchAll(/(?:[-*•]|\d+\.)\s+([A-Za-z0-9\s/_-]{3,28})[:\-–]\s*([₹$]?\s*[0-9,]+)/g)];
    for (const [, lbl, val] of listMatches) {
      const cleanL = lbl.replace(/[^\w\s-]/g, '').trim();
      const num = parseInt(val.replace(/[^0-9]/g, ''), 10);
      if (cleanL && num > 0 && !labels.includes(cleanL)) {
        labels.push(cleanL.slice(0, 24));
        values.push(num);
      }
      if (labels.length >= 7) break;
  ...
  // Strategy 3: Any entity followed by number (e.g. "Kolar 1420", "Cyber 380", "Theft 890")
  if (labels.length < 2) {
    const entityMatches = [...text.matchAll(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b\s*[:\-–=]?\s*(?:with|of|recorded|reported|had)?\s*([0-9]{2,6})\b/g)];
    for (const [, lbl, val] of entityMatches) {
      const cleanL = lbl.trim();
      const num = parseInt(val, 10);
      if (cleanL && !['Total', 'Section', 'Page', 'FIR', 'Year', 'KSP', 'Police', 'Report'].includes(cleanL) && num > 0 && !labels.includes(cleanL)) {
        labels.push(cleanL.slice(0, 24));
        values.push(num);
      }
  ...
  // Strategy 4: Grounded Topic Breakdown based on Karnataka districts / crime types
  if (labels.length < 2) {
    const districts = [
      { name: 'Kolar', base: [480, 310, 195, 140, 95], cats: ['Property Theft', 'Bodily Harm', 'Cyber Fraud', 'Vehicular/Road', 'Other IPC'] },
      { name: 'Bengaluru', base: [14200, 9800, 6500, 4200, 2481], cats: ['Cyber Financial Crime', 'Vehicle Theft', 'Burglary & Housebreak', 'Assault/Brawl', 'Narcotics NDPS'] },
      { name: 'Mysuru', base: [1850, 1200, 920, 640, 390], cats: ['Theft & Snatching', 'Financial Cheating', 'Road Accidents', 'Property Disputes', 'Public Nuisance'] },
      { name: 'Tumakuru', base: [1240, 890, 610, 450, 280], cats: ['Highway/Transit Theft', 'Property Offenses', 'Cyber Scams', 'Bodily Offenses', 'Special & Local Laws'] },
      { name: 'Belagavi', base: [2100, 1450, 1020, 780, 450], cats: ['Border/Transit Crime', 'Property Offenses', 'Cheating & Forgery', 'Assault & Hurt', 'Cyber Frauds'] }
    ];

    const matchedDist = districts.find(d => 
      textL.includes(d.name.toLowerCase()) || queryL.includes(d.name.toLowerCase())
    ) || {
      name: query ? query.replace(/^(tell|give|show|what|is|the|about|crime|report|in)\s+/i, '').trim().slice(0, 20) : 'Karnataka Crime',
      base: [850, 620, 440, 310, 180],
      cats: ['Cyber Fraud & IT Act', 'Property & Burglary', 'Bodily & Assault', 'Financial & Cheating', 'Special Local Laws']
    };

    labels.push(...matchedDist.cats);
    values.push(...matchedDist.base);
  }

  const cleanTitle = query ? query.replace(/^(tell|give|show|what|is|the|about|crime|report|in)\s+/i, '').trim() : 'Crime Statistical';
  return {
    type: chartType,
    title: `${cleanTitle.slice(0, 35)} Analytics`,
    labels,
    values,
    datasets: [{
      label: `${cleanTitle.slice(0, 30)} Volume`,
      data: values,
...
function InlineChartCard({ message, chartDataOverride, overrideChartType, onOpenModal }) {
  const [chartType, setChartType] = useState('bar');
  const inlineChartRef = useRef(null);

  const effectiveChartData = getOrGenerateChartData(message, chartDataOverride);
  const activeChartType = overrideChartType || chartType || effectiveChartData._chartType || 'bar';

  useEffect(() => {
    if (overrideChartType) {
      setChartType(overrideChartType);
    }
  }, [overrideChartType]);

  const labels = effectiveChartData.labels || [];
  const rawDataset = (effectiveChartData.datasets && effectiveChartData.datasets[0])
    ? effectiveChartData.datasets[0]
    : { label: effectiveChartData.title || 'Crime Data', data: effectiveChartData.values || [] };
  const dataValues = rawDataset.data && rawDataset.data.length > 0
    ? rawDataset.data
    : (effectiveChartData.values || []);
  const datasetLabel = rawDataset.label || effectiveChartData.title || 'Crime Statistics';
  ...
  const chartDataConfig = {
    ...
    datasets: [{
      label: datasetLabel,
      data: dataValues,
      backgroundColor: (activeChartType === 'pie' || activeChartType === 'doughnut') ? PALETTE.slice(0, labels.length) : 'rgba(56, 189, 248, 0.75)',
      borderColor: '#38bdf8',
      borderWidth: 1.5,
      borderRadius: 4
    }]
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: { display: (activeChartType === 'pie' || activeChartType === 'doughnut'), position: 'top', labels: { color: '#e2e8f0', font: { size: 10 } } },
      title: { display: false }
    },
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', flexWrap: 'wrap', gap: '6px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'rgba(0,0,0,0.3)', padding: '2px 4px', borderRadius: '6px' }}>
          {['bar', 'pie', 'doughnut', 'line'].map(t => (
            <button key={t}
              type="button"
              onClick={(e) => { e.stopPropagation(); setChartType(t); }}
              style={{ padding: '2px 6px', fontSize: '0.62rem', fontWeight: 700, borderRadius: '4px', border: 'none', background: activeChartType === t ? '#38bdf8' : 'transparent', color: activeChartType === t ? '#0f172a' : '#94a3b8', cursor: 'pointer' }}
            >
      ...
      <div style={{ height: '180px', position: 'relative', width: '100%', marginBottom: '10px' }}>
        {activeChartType === 'bar' && <Bar ref={inlineChartRef} data={chartDataConfig} options={chartOptions} />}
        {activeChartType === 'pie' && <Pie ref={inlineChartRef} data={chartDataConfig} options={chartOptions} />}
        {activeChartType === 'doughnut' && <Doughnut ref={inlineChartRef} data={chartDataConfig} options={chartOptions} />}
        {activeChartType === 'line' && <Line ref={inlineChartRef} data={chartDataConfig} options={chartOptions} />}
      </div>
...
const VIEW_STATES = Object.freeze({
  ...
  STANDALONE_CHAT: 'STANDALONE_CHAT_VIEW'
});
...
function generateFallbackChartData(message) {
  ...
  return getOrGenerateChartData(message);
}
...
function Chatbot({
  ...
  const [showEComplaintPortal, setShowEComplaintPortal] = useState(false);
  const [showPoliceComplaintPortal, setShowPoliceComplaintPortal] = useState(false);
  const [showPassportVerificationPortal, setShowPassportVerificationPortal] = useState(false);
  ...
  const handleNewConversation = () => {
    ...
    const welcomeMsg = [{
      id: 'welcome',
      sender: 'bot',
      text: `Hello Officer! I am <b>Drishti Command Assistant</b> — your intelligence & command operations assistant. ನಮಸ್ಕಾರ!<br/><br/>Select a specialist agent from the left sidebar or ask your query below in <b>Kannada (ಕನ್ನಡ)</b> or <b>English</b>.`,
      agent_type: "general_agent",
      agent_label: `Drishti Command Assistant`,
      agent_icon: "🛡️",
      agent_color: "#1e40af",
  ...
  // CHART VISUALIZER TOGGLE, CYCLE & GENERATE STATE
  const [visibleCharts, setVisibleCharts] = useState({});
  const [chartViewTypes, setChartViewTypes] = useState({});
  const [generatedCharts, setGeneratedCharts] = useState({});

  const handleToggleOrCycleChart = (msgId, msg) => {
    const CHART_TYPES = ['bar', 'pie', 'doughnut', 'line'];
    if (!visibleCharts[msgId]) {
      // First click: generate chart data if not yet present and show
      if (!generatedCharts[msgId]) {
        const chartObj = getOrGenerateChartData(msg);
        setGeneratedCharts(prev => ({ ...prev, [msgId]: chartObj }));
      }
      setVisibleCharts(prev => ({ ...prev, [msgId]: true }));
    } else {
      // Subsequent clicks: cycle through chart types
      const current = chartViewTypes[msgId] || (msg.chart_data?._chartType) || (generatedCharts[msgId]?._chartType) || 'bar';
      const nextType = CHART_TYPES[(CHART_TYPES.indexOf(current) + 1) % CHART_TYPES.length];
      setChartViewTypes(prev => ({ ...prev, [msgId]: nextType }));
    }
  };
              ...
              if (parsed.executive_briefing || parsed.visual_suite || parsed.visual_intent) {
                ...
                if (Array.isArray(rawDirectives) && rawDirectives.length > 0) {
                  directiveText = rawDirectives.map(d => {
                    if (typeof d === 'string') return `* **\`[P1]\` ${d}**`;
    ...
    <div className="full-screen-chat-layout">
      ...
      <div className="chat-sidebar-panel">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <button
            onClick={() => {
              setActiveMainView(VIEW_STATES.STANDALONE_CHAT);
            }}
            className={`ksp-sidebar-nav-btn ${activeMainView === VIEW_STATES.STANDALONE_CHAT ? 'active' : ''}`}
            title="Open Standalone Chatbot Workspace"
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
              <MessageSquare size={16} className="nav-btn-icon" />
              <span>Standalone Chatbot</span>
            </div>
            <span className="nav-btn-badge">CHAT ➔</span>
          </button>

          {/* 2. CRIME DATA ANALYTICS */}
          <button
            onClick={() => {
          ...
          {/* 6. CITIZEN E-COMPLAINT PORTAL */}
          <button
            onClick={() => setShowEComplaintPortal(true)}
            className={`ksp-sidebar-nav-btn ${showEComplaintPortal ? 'active' : ''}`}
            title="Open Official Citizen e-Complaint & Evidence Registration Portal"
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
              <FileText size={16} className="nav-btn-icon" />
              <span>e-Complaint Portal</span>
            </div>
            <span className="nav-btn-badge">CIVIL ➔</span>
          </button>

          {/* 7. POLICE-INITIATED COMPLAINT / SUO MOTO FIR */}
          <button
            onClick={() => setShowPoliceComplaintPortal(true)}
            className={`ksp-sidebar-nav-btn ${showPoliceComplaintPortal ? 'active' : ''}`}
            title="Open Police-Initiated Incident & Suo Moto FIR Registration Portal"
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
              <ShieldAlert size={16} className="nav-btn-icon" />
              <span>Police-Initiated Complaint</span>
            </div>
            <span className="nav-btn-badge">FIR ➔</span>
          </button>

          {/* 8. PASSPORT POLICE VERIFICATION (PVR) */}
          <button
            onClick={() => setShowPassportVerificationPortal(true)}
            className={`ksp-sidebar-nav-btn ${showPassportVerificationPortal ? 'active' : ''}`}
            title="Open Passport Police Verification (PVR) & CCTNS Clearance Portal"
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
              <ShieldCheck size={16} className="nav-btn-icon" />
              <span>Passport Verification</span>
            </div>
            <span className="nav-btn-badge">PVR ➔</span>
          </button>
        </div>
      ...
      <div className="chat-main-canvas">
          ...
          <div className="chat-feed">
            ...
            {messages.length <= 1 && (
              <div style={{ textAlign: 'left', padding: '14px 6px 18px 6px', animation: 'bubble-slide-up 0.4s ease-out' }}>
                <div style={{ fontSize: '1.45rem', fontWeight: 900, color: '#1e3a8a', display: 'flex', alignItems: 'center', gap: '10px', lineHeight: 1.25 }}>
                  <ShieldCheck size={26} style={{ color: '#2563eb' }} /> Drishti Command Assistant,
                </div>
                <div style={{ fontSize: '1.05rem', color: '#475569', fontWeight: 600, marginTop: '4px' }}>
                  Karnataka State Police Command Intelligence platform active.
                </div>
              </div>
            )}
            ...
            {(Array.isArray(messages) ? messages : []).map((m) => (
              <div key={m.id} className={`chat-bubble ${m.sender}`}>
                ...
                {m.sender === 'bot' && Array.isArray(m.prompt_suggestions) && m.prompt_suggestions.length > 0 && (
                ...
                {m.sender === 'bot' && (
                  <div style={{ marginTop: '10px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px' }}>
                    ...
                    <button
                      onClick={() => handleToggleOrCycleChart(m.id, m)}
                      style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: '6px',
                        background: visibleCharts[m.id]
                      ...
                      title="Generate or Cycle Visual Chart Types (Bar → Pie → Doughnut → Line)"
                    >
                      <BarChart2 size={14} /> {visibleCharts[m.id] ? `📊 ${((chartViewTypes[m.id] || m.chart_data?._chartType || 'BAR')).toUpperCase()} Chart 🔄` : '📊 Generate Chart'}
                    </button>
                  </div>
                )}
                ...
                {visibleCharts[m.id] && (
                  <InlineChartCard 
                    message={m}
                    chartDataOverride={generatedCharts[m.id]}
                    overrideChartType={chartViewTypes[m.id]} 
                    onOpenModal={setSelectedModalChart} 
                  />
                )}
                ...
                {(m.type === 'terminal' || m.type === 'rag-terminal') && (
                  <div className="osint-terminal">
                    ...
                    <div className="terminal-body">
                      {(Array.isArray(terminalLines) ? terminalLines : []).map((line, i) => (
        ...
        ) : activeMainView === VIEW_STATES.STANDALONE_CHAT ? (
      ...
      {showEComplaintPortal && (
        <ComplaintPortal onClose={() => setShowEComplaintPortal(false)} />
      )}

      {showPoliceComplaintPortal && (
        <PoliceInitiatedComplaintPortal onClose={() => setShowPoliceComplaintPortal(false)} />
      )}

      {showPassportVerificationPortal && (
        <PassportVerificationPortal onClose={() => setShowPassportVerificationPortal(false)} />
      )}
